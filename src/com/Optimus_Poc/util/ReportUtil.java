...
import com.Optimus_Poc.read.Xls_Reader;
import com.Optimus_Poc.test.Constants;

/**
 * @version MHR_v.8(Report folder overriding feature, Report folder cleaning on override)
 * @description Class generating HTML report after the execution of the Web driver - Best result with FireFox
 *              browser
 * @required The web driver should be completed the execution and the XL should contain contains the result ,M .K
 * @author MIH
 */
public class ReportUtil {
	...
	public static String resultFolderName = "Automation_Reports"; // Set the result folder name
	public static String chartTitle = "Report Overview"; // Set the Report folder name
	public static boolean folderOverride = true; // Set the overriding status
	public static boolean compressAndMail = true; // Set for compressing the Reports folder
	...
	public static String makePiGraph(int passedCount, int failedCount) {
		...
		String script = "<script type='text/javascript' src='https://www.google.com/jsapi'></script>" + "<script type='text/javascript'>"
		        + "google.load('visualization', '1', {packages:['corechart']});" + "google.setOnLoadCallback(drawChart);" + "function drawChart() {"
		        + "var data = google.visualization.arrayToDataTable([" + "['Status', 'Test Case Count']," + "['PASS',     " + passedCount + "],"
		        + "['FAIL',      " + failedCount + "]" + "]);" + "var options = {" + "title: 'Test Case Report',"
	...
	public static String makePiChartOffline(int passedCount, int failedCount, String chartTitle) {
		...
		String script = "<script type='text/javascript' src='../../libs/pi_graph/canvasjs.min.js'></script>" + "<script type='text/javascript'>"
		        + "window.onload = function () {" + "CanvasJS.addColorSet('testResult_color'," + "[" + "'#009925'," + "'#DD4B39'" + "]);"
	...
	public static void main(String[] arg) throws Exception {
		...
		try {
			...
			if (!folderOverride) {
				date = (((new Date().toString()).replaceAll(" ", "_")).replaceAll(":", "_")).replaceAll("\\+", "_");
				resultFolderName = resultFolderName + "//" + resultFolderName + "_" + date;
			} else {
				if (new File(resultFolderName).exists()) {
					FileUtils.cleanDirectory(new File(resultFolderName));
				}
			}
			...
			fs = new FileInputStream(System.getProperty("user.dir") + "//src//com//Optimus_Poc//config//config.properties");
			...
			suiteXLS = new Xls_Reader(System.getProperty("user.dir") + "//src//com//Optimus_Poc//xls//Suite.xlsx");
			...
			for (int currentSuiteID = 2; currentSuiteID <= totalTestSuites; currentSuiteID++) {
				...
				if (suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID).equalsIgnoreCase("N")) {
					continue;
				}
				testCaseResult = true;
				currentTestSuite = suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.SUITE_ID, currentSuiteID);
				currentSuiteXLS = new Xls_Reader(System.getProperty("user.dir") + "//src//com//Optimus_Poc//xls//" + currentTestSuite + ".xlsx");
				...
				outTestCases.write("<html><HEAD> <TITLE>" + currentTestSuite
				        + " Test Cases</TITLE></HEAD><body><h4 align=center><FONT COLOR=660066 FACE=AriaL SIZE=6><b><u>" + currentTestSuite
				...
				for (int currentTestCaseID = 2; currentTestCaseID <= testCaseRows; currentTestCaseID++) {
					currentTestName = currentSuiteXLS.getCellData(Constants.TEST_CASES_SHEET, Constants.TCID, currentTestCaseID);
					// Checking the run mode of the suites and skips to the next iteration if the run mode is
					// 'N'
					if (currentSuiteXLS.getCellData(Constants.TEST_CASES_SHEET, Constants.RUNMODE, currentTestCaseID).equalsIgnoreCase("N")
					        || currentSuiteXLS.getCellData(Constants.TEST_CASES_SHEET, Constants.TCID, currentTestCaseID).length() == 0) {
						continue;
					}
					System.out.println(currentTestSuite + " -- " + currentTestName);
					System.out.println("TCID " + currentSuiteXLS.getCellData(Constants.TEST_CASES_SHEET, Constants.TCID, currentTestCaseID));
					testCaseName = currentSuiteXLS.getCellData(Constants.TEST_CASES_SHEET, Constants.TCID, currentTestCaseID);
					...
					for (int innerRow = 0; innerRow < testStepRows; innerRow++) {
						if ((currentSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, Constants.TCID, innerRow).equalsIgnoreCase(testCaseName))) {
							testStepRowIndex = innerRow;
							while (currentSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, Constants.TCID, innerRow).equalsIgnoreCase(testCaseName)
							        || currentSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, Constants.TCID, innerRow).isEmpty()) {
								innerRow++;
								if (currentSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, Constants.TCID, innerRow).isEmpty()) {
									if (terminalCount > 3) {
										break;
									}
									terminalCount++;
					...
					outTestSteps.write("<html><HEAD> <TITLE>" + testCaseName
					        + " Test Results</TITLE></HEAD><body><h4 align=center><FONT COLOR=660066 FACE=AriaL SIZE=6><b><u> " + testCaseName
					...
					for (int colNum = 0; colNum < testStepCols; colNum++) {
						outTestSteps.write("<td align= center bgcolor=#153E7E><FONT COLOR=#ffffff FACE= Arial  SIZE=2><b>");
						outTestSteps.write(currentSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, colNum, 1));
					}
					...
					for (int subInnerRows = testStepRowIndex; subInnerRows <= lastIndex; subInnerRows++) {
						...
						for (int subInnerCols = 0; subInnerCols < testStepCols; subInnerCols++) {
							data = currentSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, subInnerCols, subInnerRows);
							resultCol = currentSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, subInnerCols, 1).startsWith(Constants.RESULT);
							if (data.isEmpty()) {
								if (resultCol) {
									data = "  ";
								} else {
							...
							if ((data.startsWith("Pass") || data.startsWith("PASS")) && resultCol) {
								outTestSteps.write("<td align=center bgcolor=#009925><FONT COLOR=#000000 FACE= Arial  SIZE=2>");
								resultFound = true;
							} else if ((data.startsWith("Fail") || data.startsWith("FAIL")) && resultCol) {
								outTestSteps.write("<td align=center bgcolor=#DD4B39><FONT COLOR=#000000 FACE= Arial  SIZE=2>");
								testStepResult = false;
								testCaseResult = false;
								resultFound = true;
							} else if ((data.startsWith("Skip") || data.startsWith("SKIP")) && resultCol) {
								outTestSteps.write("<td align=center bgcolor=yellow><FONT COLOR=#000000 FACE= Arial  SIZE=2>");
								skipFound = true;
							} else {
								outTestSteps.write("<td align= center bgcolor=#ffffff><FONT COLOR=#000000 FACE= Arial  SIZE=2>");
							}
					...
					for (int colNum = 0; colNum < (testCaseCols - 1); colNum++) {
						data = currentSuiteXLS.getCellData(Constants.TEST_CASES_SHEET, colNum, currentTestCaseID);
						if (currentSuiteXLS.getCellData(Constants.TEST_CASES_SHEET, colNum, 1).contains("Mapped TCID")
						        || currentSuiteXLS.getCellData(Constants.TEST_CASES_SHEET, colNum, 1).contains("Automated TC Count")) {
							continue;
						}
						if (colNum == 0) {
							outTestCases.write("<td align= center bgcolor=#ffffff><FONT COLOR=153E7E FACE= Arial  SIZE=2><b>");
							outTestCases.write("<a href=" + testCaseName.replace(" ", "%20") + "_steps.html>" + testCaseName + "</a>");
						} else if (colNum == 2) {
							if (skipFound == true && resultFound == false) {
								outTestCases.write("<td align= center bgcolor=yellow><FONT COLOR=153E7E FACE= Arial  SIZE=2><b>");
								outTestCases.write("SKIP");
							} else if (testStepResult == true) {
								outTestCases.write("<td align= center bgcolor=#009925><FONT COLOR=153E7E FACE= Arial  SIZE=2><b>");
								outTestCases.write("PASS");
								testCasePassedCount++;
								totalTestPassedCount++;
							} else if (testStepResult == false) {
								outTestCases.write("<td align= center bgcolor=#DD4B39><FONT COLOR=153E7E FACE= Arial  SIZE=2><b>");
				...
				if (!((testCasePassedCount + testCaseFailedCount) == 0)) {
					outTestCases.write(makePiChartOffline(testCasePassedCount, testCaseFailedCount, currentTestSuite + " Overview"));
				...
				// creating the link
				out.write("<a href=" + currentTestSuite.replace(" ", "%20") + "_cases.html>" + currentTestSuite + "</a>");
		...
		if (compressAndMail) {
			Zip.zip(System.getProperty("user.dir") + "/" + ReportUtil.resultFolderName, CONFIG.getProperty("report_file_name"));
